...
document.getElementsByClassName('.banner').ondragstart = function() { return false; };
...
var bannerChildren = ''
bannerDataArray.forEach(function (item, i) {
	bannerChildren = bannerChildren + renderBannerTile(item)
})
$('#certifications-wrapper')[0].innerHTML = $('#certifications-wrapper')[0].innerHTML + bannerChildren
...
function renderBannerTile(data){
	return `

		<div class="col-11 col-md-7 banner" style="max-width: 1000px;">

          <h1 class="col-12 pl-0 mb-0" style="font-size: 16px; line-height: 1.25; font-weight: 350; letter-spacing: .023em;">${data.title}</h1>
          <h1 class="m-0" style="font-size: 12px; line-height: 1.25; font-weight: 350; letter-spacing: .023em; text-align: left; align-self: left; color: rgb(50, 120, 240);">${data.subtitle}</h1>

          <div class="w-100 mb-4 mt-3" style="box-shadow: 0px 0px 0px 0 rgb(0 0 0 / 20%), 0px 5px 15px 0 rgb(0 0 0 / 15%); border-radius: 15px;">
            <img class="border" style="width: 100%; height: auto; border-radius: 15px; box-shadow: 0px 0px 0px 0 rgb(0 0 0 / 20%), 0px 5px 15px 0 rgb(0 0 0 / 15%);" src="${data.img}">
          </div>
